...
#include "../MultiGumballMachineState_refill/CMultiGumballMachine.h"
...
SCENARIO("CHasQuarterState test all functions")
{
	GIVEN("Gumball machine with 3 gum")
	{
		...
		WHEN("try to refill balls")
		{
			gumballMachine.AddBalls(5);

			THEN("gumballs will be refilled")
			{
				REQUIRE(gumballMachine.ToString() == MakeState(8, 1, "waiting for turn of crank"));
			}
		}
	}
}
...
SCENARIO("CNoQuarterState test all functions")
{
	GIVEN("Gumball machine with 3 gum")
	{
		...
		WHEN("try to refill balls")
		{
			...
			THEN("gumballs will be refilled")
			{
				REQUIRE(gumballMachine.ToString() == MakeState(8, 0, "waiting for quarter"));
			}
		}
	}
}
...
SCENARIO("CHasMaxQuarters test all functions")
{
	GIVEN("Gumball machine with 5")
	{
		...
		WHEN("try to add balls")
		{
			...
			THEN("gumballs will be refilled")
			{
				REQUIRE(gumballMachine.ToString() == MakeState(10, 5, "full of quarters"));
			}
		}
	}
}