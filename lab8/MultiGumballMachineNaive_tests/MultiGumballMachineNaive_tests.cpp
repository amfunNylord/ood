...
#include "../MultiGumballMachineNative/MultiGumballMachineNaive.h"

std::string MakeState(unsigned ballsCount, unsigned quartersCount, const std::string state)
{
	return "Mighty Gumball, Inc.\nC++-enabled Standing Gumball Model #2016 (with state)\nInventory: " + 
		   std::to_string(ballsCount) + " gumball" + (ballsCount != 1 ? "s" : "") + "\nQuarters count: " + 
		   std::to_string(quartersCount) + "\nMachine is " + state;
}
...
SCENARIO("CHasQuarterState test all functions")
{
	...
	{
		naive::CMultiGumballMachine gumballMachine(3);
...
SCENARIO("CSoldOutState test all functions")
{
	...
	{
		naive::CMultiGumballMachine gumballMachine(0);
	...
	{
		naive::CMultiGumballMachine gumballMachine(1);
...
SCENARIO("CHasMaxQuarters test all functions")
{
	...
	{
		naive::CMultiGumballMachine gumballMachine(5);
		gumballMachine.InsertQuarter();
		gumballMachine.InsertQuarter();
		gumballMachine.InsertQuarter();
		gumballMachine.InsertQuarter();
